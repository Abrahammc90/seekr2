...
def test_MMVT_anchor_id_from_alias(host_guest_mmvt_sda_model):
    """
    Test the anchor's ability to return information about itself and its
    milestones - id_from_alias.
    """
    anchor0 = host_guest_mmvt_sda_model.anchors[0]
    anchor1 = host_guest_mmvt_sda_model.anchors[1]
    # test id_from_alias
...
def test_MMVT_anchor_alias_from_id(host_guest_mmvt_sda_model):
    """
    Test the anchor's ability to return information about itself and its
...
def test_MMVT_anchor_alias_from_neighbor_id(host_guest_mmvt_sda_model):
    """
    Test the anchor's ability to return information about itself and its
...
def test_MMVT_anchor_get_ids(host_guest_mmvt_sda_model):
    """
    Test the anchor's ability to return information about itself and its
    milestones - id_from_alias.
    """
...
def test_check_openmm_context_within_boundary(host_guest_mmvt_sda_model, tmp_path):
    """
    ...
    my_sim_openmm = mmvt_sim_openmm.create_sim_openmm(
        host_guest_mmvt_sda_model, anchor, output_file)
    context = my_sim_openmm.simulation.context
    
    final_result = True
    for milestone in anchor.milestones:
        cv = host_guest_mmvt_sda_model.collective_variables[milestone.cv_index]
    ...
    anchor_pdb2 \
        = host_guest_mmvt_sda_model.anchors[1].amber_params.pdb_coordinates_filename
    ...
    anchor_pdb_src_path2 = os.path.join(
        host_guest_mmvt_sda_model.anchor_rootdir, 
        host_guest_mmvt_sda_model.anchors[1].directory,
        host_guest_mmvt_sda_model.anchors[1].building_directory, 
        anchor_pdb2)
