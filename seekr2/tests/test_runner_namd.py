...
def test_search_for_state_to_load(host_guest_mmvt_sda_model):
    anchor0 = host_guest_mmvt_sda_model.anchors[1]
    state_dir = os.path.join(
        host_guest_mmvt_sda_model.anchor_rootdir, anchor0.directory,
        anchor0.production_directory, "states")
    if not os.path.exists(state_dir):
        os.mkdir(state_dir)
    state0 = os.path.join(state_dir, "namdmmvt_2_2")
    pathlib.Path(state0).touch()
    anchor1 = host_guest_mmvt_sda_model.anchors[2]
    state1 = runner_namd.search_for_state_to_load(host_guest_mmvt_sda_model, anchor1)
    assert state1 == os.path.join(state_dir, "namdmmvt_2_2")
    anchor2 = host_guest_mmvt_sda_model.anchors[3]
    state2 = runner_namd.search_for_state_to_load(host_guest_mmvt_sda_model, anchor2)
...
def test_Runner_namd_default(host_guest_mmvt_sda_model):
    host_guest_mmvt_sda_model.calculation_settings.num_production_steps = 10
    host_guest_mmvt_sda_model.namd_settings = base.Namd_settings()
    myanchor = host_guest_mmvt_sda_model.anchors[1]
    namd_command = "namd2"
    namd_arguments = ""
    runner = runner_namd.Runner_namd(
        host_guest_mmvt_sda_model, myanchor, namd_command, namd_arguments)
...
def test_Runner_namd_load_state(host_guest_mmvt_sda_model):
    host_guest_mmvt_sda_model.calculation_settings.num_production_steps = 10
    host_guest_mmvt_sda_model.namd_settings = base.Namd_settings()
    myanchor = host_guest_mmvt_sda_model.anchors[1]
    namd_command = "namd2"
...
def test_Runner_namd_save_states(host_guest_mmvt_sda_model):
    host_guest_mmvt_sda_model.calculation_settings.num_production_steps = 10
    host_guest_mmvt_sda_model.namd_settings = base.Namd_settings()
    myanchor = host_guest_mmvt_sda_model.anchors[1]
    namd_command = "namd2"
...
def test_Runner_namd_save_states_until_all_bounds(host_guest_mmvt_sda_model):
    host_guest_mmvt_sda_model.calculation_settings.num_production_steps = 10
    host_guest_mmvt_sda_model.namd_settings = base.Namd_settings()
    myanchor = host_guest_mmvt_sda_model.anchors[1]
    namd_command = "namd2"
